...
void Header::parseHeaders( std::string &headers_string )
{
    _parser(headers_string);
}
...
const std::string &Header::getCookie( const std::string &key ) const
{
...
void Header::_header_pair_parser( const std::string &s, char del)
{
    ...
    std::string key = to_lower(s.substr(0, pos));
