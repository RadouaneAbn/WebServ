...
class Request {
	RequestLocationInfo _request_info;
    std::string _method;
	std::string _path;
	...
	static size_t n_file;

	void extract_headers_t_e( void );
	void extract_headers_c_l( void );
...
public:
	...
	void set_request_info(const std::string &root, const std::string &filepath);

	bool is_timedout( time_t time ) const;
